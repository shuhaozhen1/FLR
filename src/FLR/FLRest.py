...
def loss(beta, data):
    ...
    for i in range(n):
        ...
        for j in range(k):
            # Calculate the time interval [(j-1)/k, j/k]
            t_start = j / k
            t_end = (j+1) / k
            
            # Find the indices of the time points in T that are within [(j-1)/k, j/k]
            idx = np.where((t >= t_start) & (t < t_end))[0]
            
            # Calculate x_ij as the average of all X(T_ij)s within [(j-1)/k, j/k]
            x_ij = np.mean(x[idx])
            
            # Update the predicted value of Y for this realization
            y_pred += k**(-1) * beta[j] * x_ij
